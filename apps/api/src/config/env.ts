...
const envSchema = z.object({
  ...
  SYSTEM_ADMIN_IDS: z.string().default('admin,123456'),
  SYSTEM_ADMIN_PASSWORD: z.string().min(1).default('admin'),
  ...
  SUPABASE_PROFILE_PHOTOS_BUCKET: z.string().min(1).default('profile-photos'),
  SUPABASE_CHAT_ATTACHMENTS_BUCKET: z.string().min(1).default('chat-attachments'),
...
export const env = {
  ...
  systemAdminIds: parsed.data.SYSTEM_ADMIN_IDS,
  systemAdminPassword: parsed.data.SYSTEM_ADMIN_PASSWORD,
  ...
  supabaseProfilePhotosBucket: parsed.data.SUPABASE_PROFILE_PHOTOS_BUCKET,
  supabaseChatAttachmentsBucket: parsed.data.SUPABASE_CHAT_ATTACHMENTS_BUCKET,
