...
import type { InventoryItemDto, InventoryTransactionDto, PaginatedResponse } from '@myinventory/shared'
...
export function DashboardPage() {
  ...
  const [recentTransactions, setRecentTransactions] = useState<InventoryTransactionDto[]>([])
    ...
    async function loadDashboard() {
      try {
        const [inventoryRes, txRes] = await Promise.all([
          apiFetch<PaginatedResponse<InventoryItemDto>>('/api/inventory?page=1&pageSize=100'),
          apiFetch<PaginatedResponse<InventoryTransactionDto>>(
            '/api/transactions?page=1&pageSize=10',
          ),
        ])

        setInventoryCount(inventoryRes.total)
        setTotalQuantity(
          inventoryRes.data.reduce((sum, item) => sum + item.quantity, 0),
        )
