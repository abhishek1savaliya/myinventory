  ...
  return (
    <div className="pointer-events-none fixed bottom-20 right-4 z-50 flex w-[min(100vw-2rem,22rem)] flex-col gap-2 lg:bottom-6">
      {notifications.map((notification) => {
        const isGroup = Boolean(notification.groupId)
        const href = isGroup
          ? orgPath(orgSlug, `/chat?group=${notification.groupId}`)
          : orgPath(orgSlug, `/chat?user=${notification.partnerId}`)
        const title = notification.title ?? notification.partnerName ?? 'Chat'

        return (
          <div
            key={notification.key ?? notification.partnerId ?? notification.groupId}
          ...
          >
            <div className="flex items-start gap-3 p-3">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[var(--color-primary)] text-[var(--color-primary-foreground)]">
                {isGroup ? <Users className="h-5 w-5" /> : <MessageCircle className="h-5 w-5" />}
              </div>
